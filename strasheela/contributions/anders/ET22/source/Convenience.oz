...
%%
%% Note, this is only a copy of corresponding functor in ET31 -- only
%% a single Var differs.  As this functor is only intended for
%% testing, this doubling might be OK. However, if you change these
%% defs a lot, consider creating a more general abstraction..
%%
   ...
   proc {PlayNames Names}
      {PlayMMidiCentScore
       {Map Names fun {$ Name}
		  ...
		  in
		     {FloatToInt
		      {IntToFloat {ET22.pc Name}} * 12.0 / 31.0  % transforms in MIDI
		      * 10000.0			   % in millimidicent
		      + OneOverOne}			   % PC -> pitch
		  end}
       {MakeFileName Names}}
