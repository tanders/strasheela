...
* Strasheela examples					      :MAYBE:STRASHEELA:  
  ...
  [[file:~/oz/music/Strasheela/private/examples/TODO.muse::title%20Examples%20TODO][file:~/oz/music/Strasheela/private/examples/TODO.muse::title Examples TODO]]
...
* Strasheela Bugs							 :MAYBE:
  ...
  [[file:~/oz/music/Strasheela/strasheela/trunk/strasheela/others/TODO/01-BUGS.oz]]
...
** DONE Define setup for convenient tutorial editing		    :ARCHIVE:
...
*** Define (almost) only Oz statements

    I am also considering whether I should perhaps only use Oz statements (Oz expressions are very unusual at the top-level and could confuse readers), and simply insert results such as score pitctures and MP3 files manually into the Org file. Executing the corresponding code block would generate these files "behind the scene", and their paths would be relative to the Org file. I would then simply insert the same pathnames manually into the Org file. Doing so would ensure that (except for the paths) the code could be used as-is by users. If the definition of the example changes, all output can be conveniently updated just by executing all code blocks again (i.e. all path names stay the same).

*** Use Literate Programming (almost) only for top-level definitions

# I would often like to only automatically execute those code
# blocks that actually define my program (without any output), but not
# the code blocks that interactively use this program (e.g., the code
# blocks that create the actual output). The reason is that each of the
# latter can take a long time to run (blocking Emacs completely for that
# time!) and I might only need to execute, say, the last of these which
# I just newly defined. So, how could I adapt org-babel-execute-buffer
# and org-babel-execute-subtree such that they skip any code block to
# which some special header argument is given (e.g., :interactive t).

    You could use Org-babel's :noweb support for defining the top-level definition only. Then you can both "execute" (feed) Oz programs from within Org buffers and also for export files. 
    You could consider exporting the actual program and solver calls into different files (using two top-level noweb code blocks).

---


# there is the ability to tangle code blocks, if you assign each
# block in the same session the same tangle header argument then they will
# all be exported to the same file by the `org-babel-tangle' command.

# I would assign different :tangle arguments to the interactive and
# non-interactive code blocks.  This will tangle them to different files,
# and from there it should be easy to use them separately.

Ah, when using the same tangle header argument for multiple code blocks then code blocks will be appended in the resulting tangled file? 


    ----

    Tonight I further thought how I could simplify the explanation of my Oz/Strasheela examples without actually using Literate Programming constructs. Oz allows a relatively flexible order of "code blocks" anyway (e.g., a function can be used before it is actually defined, because function names are logic variables), so simply having a session would actually be sufficient for most cases (which BTW is rather similar to the quasi Literate Programming support buildin in Haskell). This line of thoughts motivated my question. When using noweb syntax only for the top-level definition, then I could also hide this top-level definition in the woven output, so readers do not need to know about Literate Programming at all. 
...
** Strasheela tutorial
   ...
   see [[id:370F385D-CA8D-4094-9374-C719975382C8][file:~/oz/music/Strasheela/StrasheelaTutorial/doc-source/StrasheelaTutorial.org]]

*** Move output from wiki into [[file:~/oz/music/Strasheela/StrasheelaTutorial/doc-source/StrasheelaTutorial.org]]
...
** Get Oz syntax highlighting at GitHub
...
** !! Gradual freezing of solution (Orjans idea)     

   Specify number of notes (or variables??) to be frozen (counted from "left to right")

   Unify these notes/variables of previous solution with corresponding notes/variables of new partial solution 

   Note: 
   - This approach requires left-to-right distro strategy (which continues working partial solution)
   - I cannot use existing methods like collect to access notes in question (collect accesses depth-first, not left-to-right)
   - Previous solution must be stored outside of script, and should be fully determined (otherwise warning) -- all unified variables outside script must be determined! 

   Todo:
   - Try to define note accessor function that makes use of existing distribution order function definitions, then I can also use different variable orderings strategies 


** Parallel search

*** Parallel search in PWGL

    Multiple searches are executed in parallel. If some search is not as efficient as specified, then it will be stopped/killed.

    Default required efficiency (user-definable)
    60 sec: 50 % of variables
    120 sec: 75 % of variables
    360 sec: 95 % of variables

    All (individual ?) search processes can also be killed manually


*** Strasheela implementation

    - Define multiple randomised search processes. 
    - Somehow, distribution process checks whether some condition is met (i.e. how many variables are determined), and this checking is somehow time-dependent.
    - Some visual feedback to show how far each process advanced (some TK window?)
    - Once this is working, try to redefine it as distributed program, so multiple processors are used
    

** Continue Fomus export revision

*** See todo in Output.oz file 

     [[file:~/oz/music/Strasheela/strasheela/trunk/strasheela/source/Output.oz::]]


** Define further transformer functions for transformable motifs

   see [[file:~/oz/music/Strasheela/strasheela/trunk/strasheela/contributions/anders/Segments/source/TransformableSubscript.oz::Transformers]]


**** ?? add pauses to motifs

**** OK Repetition

     not a variation of course, but an important technique

     Also, if harmony changes a lot then you might consider to keep rhythm more simple..

**** Replace a (longer) duration by multiple shorter durations (of equal length ?) that substitute the longer dur

**** OK Replace a duration by an arbitrary other rhythm

     in principle, this replacement rhythm does not even need to have the same duration as the duration it substitutes
     
**** Add an arbitrary short or long note 

     Where?

**** Remove an arbitrary note
...
**** Change duration of a note
     ...
     - change dur of longest or last note of motif longer

     - reduce duration of some short note

     - add a dot to some note, or remove a dot

**** Special technique from Tala in Carnatic music
     
     http://en.wikipedia.org/wiki/Tala_%28music%29#Tala_in_Carnatic_music

     Multiple duration categories 
     - 1 pulse
     - 2 pulses
     - multiple pulses: 3, 4, 5, 7 or 9

     So, a rhythm can be streched/compressed by changing dur of all long tones


**** OK Stretch/compress by multiplying all durations with some constant factor 

**** OK Stretch/compress by adding some constant value to all durations 
     
**** OK "Tilted stretching/compressing": add/multiply different durations 

     e.g., multiply all durations with an increasing list of durations 

     Note: this technique is limited when using integer durations only


**** Reverse the duration sequence

**** ?? anything like a rhythmic inversion? 

     does that have perceptual relevance?

**** Rotation of the duration sequence 

     e.g., last value becomes first etc

     Note that for motifs that end on a long note (convincing phrasing), this technique can have unconvincing results

**** Successivly apply multiple of the techniques above to by and by change motif 
        

** Allow effectively for negative offset times of items in a sequential and simultaneous containers 

   Purpose: extend my homophonic slices such that overlapping notes are possible (e.g., upbeat)

   I would need to extend temporal containers (simultaneous, but for consistency also sequential) such that effectively negative offset times of its contained items are supported 
   
   Implementation: add a parameter to temporal container called, e.g., "offsetCorrection"
   By default it is 0. 
   - in a simultaneous, its value is subtracted from the start time of every contained item
   - in a sequential, its value is subtracted from the start time of the first contained item

  As this parameter is usually determined (i.e. 0), it is filtered out by distribution strategies

  NOTE (add to doc of simultaneous/sequential): 
  Negative start times must be avoided, e.g., by placing some rest before first temporal container.
  If offsetCorrection is \= 0, then the offsetTimes of all contained items should be explicitly specified. Their default 0 then means that they start *before* their container 

  

** TODO [#A] Interval class and Interval vector

   HS.score.transposePC is ordered pitch-class interval, do also unordered pitch-class interval (interval class)

   http://en.wikipedia.org/wiki/Interval_class

   see [[file:~/oz/music/Strasheela/strasheela/trunk/strasheela/contributions/anders/HarmonisedScore/source/Score.oz::proc%20TransposePC%20UnTranspPC%20TranspositionPC%20TranspPC][file:~/oz/music/Strasheela/strasheela/trunk/strasheela/contributions/anders/HarmonisedScore/source/Score.oz::proc TransposePC UnTranspPC TranspositionPC TranspPC]]


   Using the unordered pitch-class interval (interval class) I can then define the Interval vector (the intervalic content)
   http://en.wikipedia.org/wiki/Interval_vector
   and the relation between an Interval vector and a pitch class set. 

   Interval vectors can be useful to def concisely a more abstract property of a chord
   I should consider introducing a pitch class set that is not restricted to a chord/scale index for this purpose..


   However, PC sets of the same Interval vector can have very different musical characters, and not all music theorists feel that the relationship is musically significant.
 

** Check set theory concepts: what can I use from them?

http://en.wikipedia.org/wiki/Set_theory_(music)
see also the "See also" section and the "External links"
...
** Add new Jazz harmony chord & scale database: port from Impro-Visor

   see beginning of [[file:/Applications/improvisor400/vocab/My.voc::scale]]

** Create Jazz motif database: port from Impro-Visor 

   see many many def of cells, idioms, and licks in
   [[file:/Applications/improvisor400/vocab/My.voc::cell%20notes%20b4%20ab8%20gb8%20eb8%20name%20blues%20chords%20Ab7][file:/Applications/improvisor400/vocab/My.voc::cell notes b4 ab8 gb8 eb8 name blues chords Ab7]]

   see simple motifs in 
   [[file:/Applications/improvisor400/vocab/My.grammar::rule%20M4%20A4%200%2001][file:/Applications/improvisor400/vocab/My.grammar::rule M4 A4 0 01]]

** Create Jazz motif database with machine learning by using Impro-Visor's resulting grammar files

   see 
   file:/Applications/improvisor400/vocab/*.grammar

   and grammars I may create myself


>>   I meanwhile found your grammar format largely explained in your SMC 2007 + 2009 paper. Nevertheless, it might be interesting for users (including myself) to have a full reference documentation of any file format associated with improvisor, in particular the format *.grammar but also *.voc and *.sty, and possible *.prefs. BTW: *.solist is a binary format, right? The *.ls format appears to be documented already.

> The .sty is documented in the style editor help window, although it needs updating. The .voc is kind of by example right now, but I will write up something for you. Similar for prefs.
...
** Develop new expressive patterns					 

   A number of ideas for new pattern constraints 

   E.g., UseMotifs pattern generalises cycle pattern by introducing multiple alternatives. Can I generalise or variate other pattern constraints similarily. Goal is to create "interestingly" shaped sequences of FD ints and FD int combinations (e.g. pairs) 
...
*** !! Pattern [skyline]
    ...
    !!?? NOTE: skyline of pitches together pitches, can be used to constrain intervals between melodic peaks


*** Pattern {AllowedIntervals Xs Intervals}

    Xs (FD ints) is constrained such that intervals between consecutive vars are only values in Intervals (list of FD ints)
    Note: Intervals need offset or are considered in both directions 

    Variant: reified version with arg for HowManyTrue

    This constraint is inspired by Jacopo B Schilingi
    
    
*** Variant of Contour with given [anchor points]

    in addition to the direction of intervals, the absolute value of certain pattern elements is given or constrainable. E.g., the absolute value of [turning points].

    Relaxed version: directions only constrained between these given/constrainable [anchor points] and their direct neighbours (i.e. other interval directions are unconstrained)

    This constraint is inspired by F Pachet
...
*** Variant of CM heap pattern

    from a given set/list of FD ints select n elements in an arbitrary order, then the next n elements etc
    (remember repetitions/variations e.g., of pitches in Morton Feldman compositions)
...
* General Strasheela development

** ??? Consider moving from svn to bzr

   not well integrated with Trac at sourceforge so far
...
*** Translate svn repository

    installing respective software on MacOS was unsuccessful
    better try latest version of bzr-svn on Linux next time

**** Email trying to get help..

     	From: 	  torsten.anders@plymouth.ac.uk
	Subject: 	subvertpy installation problem on Mac (was: Bug of bzr branch)
	Date: 	19 March 2009 23:16:21 GMT
	To: 	  jelmer@vernstok.nl

Geachte Meneer Jelmer,

Jelmer Vernooij wrote:
This particular branch works fine here with bzr-svn 0.5.3 and bzr  
1.19.
Please try again with a newer version of bzr.

Thank you very much for your quick reply, and for even testing my  
subversion branch. I will try a newer version of bzr-svn / bzr.

My actual problem is that I spend hours trying to install bzr-svn on  
Mac OS X (10.4) without success, I was unable to install its  
dependency subvertpy. I am telling you this only, because it appears  
you are the author/maintainer of subvertpy.

The problem is that I was unable to find a Mac version of the  
development files of subversion (what on Linux would be something  
like libsvn-dev), required for compiling subvertpy. I also tried the  
Macports of py25-subvertpy without success -- the compilation fails  
again because the subversion development files are missing (so, there  
seems to be a bug in the subvertpy port file -- perhaps a dependency  
is missing?).

Any help would be highly appreciated!

   (later I tried bzr-svn using my old Linux box, which resulted in  
the error reported previously ...)
...
in run
  ...
  File "/usr/lib/python2.5/site-packages/bzrlib/plugins/svn/ 
format.py",
line 84, in sprout
...
*** Check workflow for bzr

    how easy is check in etc
    
    how is integration with Sourceforge software (e.g. Trac)
    seems currently non-existant, see

    https://sourceforge.net/tracker/index.php?func=detail&aid=2778613&group_id=1&atid=350001
    https://sourceforge.net/tracker/index.php?func=detail&aid=2795953&group_id=1&atid=350001

    https://sourceforge.net/apps/trac/strasheela/admin
