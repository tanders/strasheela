...
#+begin_src oz :results output 
  ...
  %% MakeTextureConstraint returns a texture constraint, a procedure with the following interface.
  
  {MyTextureConstraint LeadingPart DependantPart Args}
  
  %% A texture constraint applies a Dependency (a constraint, see below) between certain notes in a LeadingPart (a container) and certain notes in a DependantPart (a container). 
  
  %% A Dependency is a procedure with the following interface.
  
  {MyDependency Note1 Note2 Args}
  
  %% A Dependency defines a constraint between Note1, a note from the LeadingPart, and Note2, a note from the DependantPart. By default, Note1 and Note2 are simultaneous notes (see the argument offsetTime below for other cases). For example, homophony can be defined by constraining that the start times and durations of Note1 and Note2 are equal. Constraints that require more complex score contexts (e.g., the note succeeding Note1 in LeadingPart) are defined by accessing such contexts from the given notes (e.g., using methods like getTemporalSuccessor). The Dependency argument Args contains values for all optional arguments in the Args argument of a texture constraint (see below). 
  %% Various dependencies are predefined (e.g., Homophonic, and HomoDirectional), and users can freely define their own.
  %%
  ...
  fun {MakeTextureConstraint Dependency}
     proc {$ LeadingPart DependantPart Args}
        Defaults = unit(numericRange: nil
                        offsetTime: 0)
         ...
         in
            thread
               ...
               = {N1 getSimultaneousItemsOffset($ As.offsetTime
                                                test: fun {$ N2}
                                                         {N2 isNote($)} andthen
                                                         {Member DependantPart
                                                          {N2 getContainersRecursively($)}}
                                                      end)}
            end
            thread
               {ForAll DependantNs
                proc {$ DependantN} {Dependency N1 DependantN As} end}
            end
         end}
     end
  end
...
#+begin_src oz :results output
  <<MakeTextureConstraint>>
    ...
    {SDistro.exploreOne 
     ...
     in
        MyScore = {Score.make sim([seq(%info: fomus(inst: 'violin')
                                       handle:Voice1
        ...
        {MyTextureConstraint Voice1 Voice2
         % unit(timeRange: [0#100])
         unit(numericRange: 1#9
              offsetTime: 2) 
        }
        {MyTextureConstraint Voice1 Voice3
         % unit(timeRange: [0#100])
         unit(numericRange: 1#9
              offsetTime: 4) 
        }
        %% voice imitates itself later
        {MyTextureConstraint Voice1 Voice1
         % unit(timeRange: [0#100])
         unit(numericRange: 1#9
              offsetTime: 6) 
        }
