...
** Texture constraints
   ...
   Texture constraints restrict the independence between parts/voices. In the Strasheela implementation, a texture constraint is applied to (a section of) one part/voice (the ), a specification of the dependent parts (used to access notes in the dependent parts), and a specification of the actual dependance. Dependence examples are homorhythm (simultaneous notes have the same start time and duration), heterorhythm (simultaneous notes have similar start times and durations), contrarhythm (simultaneous notes have different same start times or durations), homodirectional etc.

Texture constraints are inspired by Berry, Wallace (1987). Structural functions in music. Courier Dover Publications.


*** Overall constraint format

#+begin_src oz :results output silent :exports none
  
  declare
  
  %%
  %% BUG:
  %% - offset time not working yet 
  %%
  ...
  %% - Consider reducing to only a single numeric range (not a list of it), at least as an option
  %%
  %% - Doc
  %% - Revise arg names
  fun {MakeTextureConstraint Dependency}
     proc {$ LeadPart FollowingPartTest Args}
        %% TODO: add default args
        As = Args
     in
        {SMapping.forNumericRange {LeadPart collect($ test:isNote)}
         As.numericRange 
         proc {$ N1}
            DependantNs
         in
            thread
               DependantNs
               = {N1 getSimultaneousItemsOffset($ As.offsetTime
                                                test: fun {$ N2}
                                                         {N2 isNote($)} andthen
                                                         {N2 findContainerRecursively($ {GUtils.toFun FollowingPartTest})} \= nil
                                                      end)}
            end
            thread
               {ForAll DependantNs
                proc {$ DependantN}
                   if DependantN \= nil %% simply ignore non-existing notes (e.g., if offset ranges beyond number of notes)
                   then {Dependency N1 DependantN}
                   else {GUtils.warnGUI "MakeTextureConstraint: non-existing note constrained."}
                   end
                end}
            end
         end}
  ...
  % fun {MakeTextureConstraint FollowingPartTests Dependency}
  %    proc {$ LeadPart Args}
  %       %% TODO: add default args
  %       As = Args
  %    in
  ...
  %        proc {$ N1}
  %           thread 
  %              %% NOTE: sim notes only returned after all sim notes are know.
  %              %% Too late for constraining rhythmic structure?
  %              SimNs = {N1 getSimultaneousItems($ test:fun {$ X}
  %                                                         {X isNote($)} andthen
  %                                                         {Some FollowingPartTests
  %                                                          fun {$ Test}
  %                                                             {X findContainerRecursively($ {GUtils.toFun Test})} \= nil
  %                                                          end}
  %                                                      end)}
  %           in
  %              {ForAll SimNs
  %               proc {$ SimN} {Dependency N1 SimN} end}
  %           end
  %        end}
  %    end
  % end
  ...
  %% - doc
  %% - revise proc name
  proc {HomoPhonic N1 N2}
     {N1 getStartTime($)} = {N2 getStartTime($)}
  ...
  proc {HeuristicHomoPhonic N1 N2} 
  ...
  proc {HierarchicHomoPhonic N1 N2}
     Start1 = {N1 getStartTime($)}
  ...
  = {MakeTextureConstraint 
     %% NOTE: for constraining direction I would need access to note pairs per part
     %% Possible solution: have a second version of MakeTextureConstraint for that case
     %% Alternative solution: access successor from within this proc
     proc {$ N1 N2} 
        {HomoPhonic N1 N2}
        % {HierarchicHomoPhonic N1 N2}
        % {HeuristicHomoPhonic N1 N2}
        {HomoDirectional N1 N2}
     end}
  ...
  {SDistro.exploreOne 
   proc {$ MyScore}
      Voice1
   in
      MyScore = {Score.make sim([seq(info: voice1
                                     handle:Voice1
      ...
      {MyTextureConstraint Voice1
       fun {$ C} {C hasThisInfo($ voice3)} end
   % unit(timeRange: [0#100])
       unit(numericRange: [1#9]
            offsetTime: 0) % TODO: avoid need for negative numbers...
      }
   end
