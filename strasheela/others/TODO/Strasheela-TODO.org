...
* TODO Update Strasheela installation instructions 		  :IMPORTANT:

  Already updated where to find sources

  Still missing: where to find selection constraints now
  [[file:~/oz/music/Strasheela/strasheela/strasheela/doc-source/Installation.muse::**%20Selection%20Constraints][file:~/oz/music/Strasheela/strasheela/strasheela/doc-source/Installation.muse::** Selection Constraints]]

  Wait for response at mailing list
  https://groups.google.com/forum/#!topic/mozart-users/sleqosv6jfE

  If that is sorted then rebuild the doc (installation instructions) and upload it

* Other programming projects

** Real-time constraint programming in Lisp (or other high-level languages)

   See also [[*Brainstorming%20--%20alternative%20platforms%20for%20Strasheela][Brainstorming -- alternative platforms for Strasheela]]

   Strasheela re-implemented in some variant of Lisp were realtime is possible, and where I can interface some other computer music system, e.g., PD or SuperCollider 

   Improvement to current Strasheela situation: Lisp is known by more potential users


*** !? Max -- best for attracting audience

    Note: Bach project (http://www.bachproject.net/) already provides some simple constraint programming (constraining a list of values with plain backtracking search -- far from efficient) 
    -- seemingly there would be an audience for that.

    Difficulties with concurrency in Max. This may be easier in SuperCollider (and Oz, and Clojure...)

**** Pro
     - Large community of users, but not necessarily developers
     - Nice free score editor developing with Bach project
     - ? JavaScript support could help as language 


**** Cons
     - Development likely slowed down by programming in lower-level language (C)
     - I would not like that language too much...


*** SuperCollider with Gecode interface

    - UGens in SuperCollider (i.e., for the server) are written in C++

    - Primitives of the sclang that start with an underscore _ are part of some classes. They are defined in C++


**** Advantage/disadvantage of using SC
     + The SC users are coders, so more likely that this Strasheela re-implementation is used by someone else using coding
     + Nice realtime music environment

     
     - Not all SC users are interested in algorithmic composition, and few will be interested in using constraint programming
     - No readily available music notation interface nor editors
     - Creating an application with a GUI for other users takes quite some time


*** Overtone (in clojure) has a nice set of music representation functions already built int (like SuperCollider as well)

    Advantage: could be an academic collaboration with someone reasonably nearby (University of Cambridge)

    See video http://vimeo.com/22798433
    http://overtone.github.io/
    
    Downside: this is again a system for experienced programmers, and there is only a small audience
    (claims to have hundreds of users, though https://groups.google.com/forum/#!forum/overtone
    and several contributors https://github.com/overtone/overtone/graphs/contributors)
    -> But the users would be the right kind of audience of something like Strasheela..
...
*** ? Use GeLisp
    ...
    Can GeLisp support Gecode 4, so that I can define problem-specific variable orderings?
    Currently it at least supports Gecode 3.2.1 (version from 2009-11-30)
    However, it only wraps the functionality of Gecode 1, [[message://3BED73F9-B01B-45AC-825C-37992698B3A3@plymouth.ac.uk][Re: Gelisp]]
    See also http://cic.javerianacali.edu.co/wiki/lib/exe/fetch.php?media=grupos:avispa:papers:tor09b.pdf
    

*** !? Compile Gecode with SWIG

    SWIG supports Common Lisp with CFFI, http://www.swig.org/compat.html 

    If Gecode could be compiled with SWIG, then I could also use some other language...

    No C++11 supported yet (not even fully previous standard?)


*** Some Lisp with interface to C (easy use of Gecode within Lisp)
...
**** ? Clozure CL
     ...
     However, what about C++ for Gecode?
     Seems to be possible in principle (e.g., I saw Qt bindings for Clozure CL)

***** C 
      OpenMCL uses an automated system that parses the C header files and generates an interface database 
      http://trac.clozure.com/ccl/wiki/OpenMclFfi
      However, as this page also explains, calling C functions requires that all their args are values that C can handle (e.g., Lisp strings and arrays first must be transformed).

***** Cocoa and Objective-C

      http://trac.clozure.com/ccl/wiki/Cocoa
...
**** ? ECL
     ...
     /yet another open-source solution that suffers from a documentation problem -- tutorial missing./


     minimal example of embedding Lisp in a C program
     http://vwood.github.io/embedded-ecl.html


     Can embed C within Lisp and compile to C

     http://ecls.sourceforge.net/index.html


     Embedded Qt Lisp (ECL embedded Qt binding, embeddable in Qt)
     http://password-taxi.at/EQL


     Not really efficient, compare
...
**** libPD
     ...
     Some lisp app can send messages (bang, int, float, and symbol messages, as well as lists and typed messages) to a running Pd patch. Print messages from Pd are sent to Lisp in return.

     This also allows to integrate Pd in other apps in various languages (Java, C++, iOS...)

     http://www.youtube.com/watch?v=xLZL6er2v-k&feature=youtu.be
     http://libpd.cc/
     https://github.com/libpd/libpd
     https://github.com/libpd/libpd/wiki

Pd on iOS, Android...      http://createdigitalmusic.com/2010/10/libpd-put-pure-data-in-your-app-on-an-iphone-or-android-and-everywhere-free/


**** Live coding with Common Lisp

     http://www.youtube.com/watch?v=xzTH_ZqaFKI

**** Fluxus and Fluxa -- Scheme-based --  allows you to quickly make live animation and audio programs, and change them constantly and flexibly
     
     http://booki.flossmanuals.net/fluxus/_draft/_v/1.0/introduction/
     http://booki.flossmanuals.net/fluxus/_draft/_v/1.0/fluxa/
...
* Brainstorming -- alternative platforms for Strasheela

  See also [[*Other%20programming%20projects][Other programming projects]]
 
** MiniZinc or Zinc

*** Relation to Strasheela
    
    Not suitable for modelling directly in MiniZinc
    - Not high-level enough, e.g., limited data structure, no functions, no higher order programming

   Can creating a MiniZinc program automatically by another language be an option 
   Difficulties: 
   - Score contexts that are undetermined before the search starts I can handle in Strasheela with delayed constraint application -- can I do that in MiniZinc?? No higher-order programming and limited data structures, so not really an option.
     - I could "compile" higher-order programming in some other language into loops in MiniZinc?
   - Could relation constraints help here?
  

*** Info

    language for modelling constraint satisfaction and optimisation problems
    - MiniZinc can be can be flattened into FlatZinc (http://www.minizinc.org/), which is supported by several constraint solvers, e.g., Gecode 
    
    http://www.g12.csse.unimelb.edu.au/zinc/doc/html/zinc-spec.html (explains differences between larger Zinc and its subset MiniZinc in appendix C). 


    !! User-definable search heuristics supported by Gecode FlatZinc interpreter 
    https://lirias.kuleuven.be/bitstream/123456789/319698/1/schrijvers_2011_0_search.pdf
    http://www.gecode.org/flatzinc.html

    Note: Zinc is very limited higher-order and MiniZinc strictly first-order -- for constraint application I really want higher order...

    Zinc
    http://g12.research.nicta.com.au/zinc_current/index_home.php

    Does MiniZinc have any abstraction facilities (e.g., functions)? Seemingly those are only part of Zinc, see also 
    http://www.hakank.org/constraint_programming_blog/2010/09/a_first_look_at_g12_zinc_basic_learning_models_etc.html
    Anyway, MiniZinc users can define predicates

    MiniZinc IDE
    http://g12ide.cecs.anu.edu.au/

**** Pages with more links etc

     http://www.hakank.org/minizinc/

     http://www.hakank.org/minizinc/index_zinc.html
...
** Python -- I would depend on Denys Duchier's maintenance of Gecode (perhaps as only user!)

   - Mainstream language
   - Reasonably high-level language 
   - Sage (http://www.sagemath.org) -- could that be helpful for modelling?


*** TODO Music software in or supporting Python

    Overview: http://wiki.python.org/moin/PythonInMusic


**** Music theory modelling 

***** ! music21 -- Python-based toolkit for computer-aided musicology

      Based on Humdrum (how?)

      http://web.mit.edu/music21/
      By Michael Cuthbert, Christopher Ariza (main developer) and several contributors, primarily contributing music databases (http://web.mit.edu/music21/doc/html/about.html)
      Main author Michael Scott Cuthbert is a musician, does not even know the Python universe extremely well it seems (but fair enough, that universe is rather big)

      Relatively comprehensive music representation in Python (hierarchical, stateful, rather strait forward).
      I would be able to use that rather well (after learning all the functions/methods etc. :)
      It adds to that 
      - Imports various formats: MusicXML, Kern (from Humdrum), MIDI, ABC. Exports to MusicXML, MIDI and Lilypond. 
      - Buildin access to several corpora of music (http://web.mit.edu/music21/doc/html/referenceCorpus.html)
      - A range of analysis graphics that can be created (statistics etc)


      It seems there is a community of users (and they are pointing out bugs, so that helps to clean the system :)
      Users includes Dmitri Tymoczko 
      https://groups.google.com/forum/?fromgroups#!forum/music21list
      Question -- why could this software develop a community, even though its interface is a programming language and it does only plain symbolic music processing? Possible answers:
      - Python relatively easy to use and widely used?
      - Rather simple design overall? Programs can consist of rather little chunks and be created step by step (in contrast to my CSP defs, which are often pretty large)
      - It answers an actual need?


      *Its analysis features could be useful for the automatic generation of compositional rules..*


***** mingus

      Looks reasonably substantial, but extremely conservative
...
***** Sebastian
      ...
      https://github.com/jtauber/Sebastian
      


*** Constraint libraries in Python

    Gecode seems by far the best candidate

**** ! Gecode bindings for Python

     How much time does Denys Duchier have to maintain this library? He seems to be the only one working on in.
     There are no bugs reported in the system, so likely few users..

     https://launchpad.net/gecode-python

     - Comprehensive: rich set of domains and constraints
     - High-level control over search process
     - High performance
...
**** or-tools
     ...
     The standard way to get the code is to checkout the source and build it from scratch. 
     -> So likely, not a big community? They say they use it internally at Google, hm..




**** Optimisation libraries build into Sage
...
**** Emma -- commercial						    :ARCHIVE:

     http://www.eveutilities.com/products/emma

     - Only integer domain -- does not say anything
     - more than 60 predefined constraints (that does not sound much)

**** python-constraint						    :ARCHIVE:
...
**** Pro
     ...
     - Appropriate user-base already there (although small)
     - Several sister projects there (can be helpful, or not...)
     - Music environment already there
     - Some development already in place for gecode libraries
     - High-level language (though compared with Oz I loose realtime support)


**** Cons

     - Closed system, and depending on commercial development tools (LispWork) -- difficult to build a community there


** SuperCollider

   - Suitable community (interested both in programming and music)
   - Nicely high-level language
...
** Max
    ...
    Note: Bach project (http://www.bachproject.net/) already provides some simple constraint programming -- seemingly there would be an audience for that.
...
** OLDER: Strasheela - Gecode interface (e.g., in new Mozart virtual machine)

   User-defined branching difficult, but (hopefully) doable
   see also Denys Duchier's email [[message://87haxt4lyd.fsf@univ-orleans.fr][Re: gecode bindings]]
   

** Comet -- commercial						    :ARCHIVE:

   Clearly business focussed -- I do not need that at all. 

   Athough it is possible to get 30 days evaluation copy of COMET, there is no information on company web page regarding the cost of the software. (Free academic license, but not free beyond academics.)
   COMET language was licensed to Dynadec in 2009.

   !! The licence would be a severe problem for an open-source project! 

   http://en.wikipedia.org/wiki/Comet_(programming_language)
   http://dynadec.com/technology/

   - Constraint programming (propagation) and local search
   - Only finite integer domain?
   - Reified constraints
   - Search can be controlled
   
   Book -- manual for commercial software named COMET
   [[http://www.amazon.co.uk/Constraint-Based-Local-Search-Pascal-Hentenryck/dp/026251348X/ref=sr_1_1?ie=UTF8&qid=1370519978&sr=8-1&keywords=Constraint-Based+Local+Search+by+Pascal+Van+Hentenryck+and+Laurent+Michel][At amazon UK]]   
   As Amazon.com -- rel negative reviews (400-page infomercial...)
http://www.amazon.com/Constraint-Based-Local-Search-Pascal-Hentenryck/dp/0262220776/ref=sr_1_1?ie=UTF8&s=books&qid=1239993819&sr=8-1
...
** TODO Define ENP score transformations

   Work in progress: [[file:~/PWGL-User/ENP/plug-ins/test-script.lisp::(in-package%20:ccl)][file:~/PWGL-User/ENP/plug-ins/test-script.lisp::(in-package :ccl)]]

   Do what you can in ENP and what is too hard in Strasheela ([[*%5B#A%5D%20Define%20Strasheela%20score%20transformations%20(instead%20of%20plain%20generation%20as%20so%20far)][Define Strasheela score transformations (instead of plain generation as so far)]]) 

*** Transformations

    Transformation examples 
    
**** Doc of note slots settable by setf

ALT-X-OFFSET [+-float] offset from the note head
ATTACK-P [NIL/T] in a chord that's tied individual notes can be tied or not. when attack-p = t the note is not tied to the previous note.
CHAN [1-128]
CLEF-NUMBER [NIL/1--x] the nth clef of the staff, e.g., in a piano staff rigth hand is 0 and left is 1
COLOR [NIL/:blue/:antiquewhite] naming follows the X11/Lispworks color name scheme
COMMENT [NIL/string] for chord editor
CONSTRAINED? [NIL/T] for constraints
DRAW-ALTERATION-P [T/NIL]
ENHARMONIC [NIL/:flat/:sharp] a hint (need to call update-note after setting the value)
INTONATION [NIL/cents]
MIDI [0-x] float - the pitch!
NOTE-HEAD [NIL/keyword] (see the list in the note menu - can be user definable)
OFFSET-DUR [NIL/+-float] time that's added to the duration after the automatic time calc.
OFFSET-TIME [0/+-float] same as above, but for the duration
VEL [0-127]
VISIBLE-P [T/NIL]

***** Expressions

In a script you'd use =(add-expression <class> <objs> <args>)=, e.g., 

=(add-expression 'slur ?1 ?2 ?3 :color :red)=

there are some examples in the Analysis section of the Tutorial.

BTW, most of the time you'd add the expression to chords instead of notes. Therefore, the beginning of your script would say:

=(* ?1 :chord (m ?1 :complete? t) (?if …. ))=
...
** TODO CSP generator/template, also for PWGL interface

   *Major problem to solve first:*
   How to define reasonably simple and powerful constraint applicators? More specifically, how to nest applicators?

   Rel simple definition, but powerful

   Note: it turns out that the generator is more simple to use for simple cases, but also allows quite complex cases, where its use becomes more advanced. I think this is OK -- I want to also cater for very advanced users :)
    

*** Result

    Parameterised search script CSP
...
*** Args
    ...
    Two args: Score, Constraints

**** Score

     Score is either 
     
     - A single score, either an object or in textual form
       (Internally, a copy of this score is created so that all its variables are local to the search script. Note that constraints already applied to the score will therefore not be retained -- see below for other arg option how to do that.)

     - A list of scores, either objects or in textual form
       The resulting score is a simultaneous container containing all the given scores (offset times can be given to the scores). Again, internally copies of the given scores will be created

     - A function returning a score 
       In this case, the score is created internally to the CSP (no copy must be created), and all constraints applied to the score in the function are retained.
       Arguments expected by the function can be given to the resulting parameterised script (TODO doc)

     - A pair F#Args, where F is a function as described above and Args is a record with the default arguments for this function of the resulting parameterised script. In other words, CSP variants can be created by specifying different arguments to the resulting script.

     - A list or record of functions, or Function#Args pairs
       The resulting score is a simultaneous container containing all the scores returned by the given functions. Args are again the default arguments.
       When calling the resulting parameterised script later, arguments for specific functions are specified using the same record features. For example, the score argument of the generator received the record 
         unit(soprano: F1#Args1 bass:F2#Args2)
       then the features soprano and bass are supported arguments of the resulting parameterised script. 
       If a list instead of a record is given, then this is internally automatically translated into a tuple, so arguments are simply positional.

       TODO: define for doc example how to call resulting parameterised script with such args.
     

**** Constraints 
     
     Constraints is a record or list of constraints to apply, where each constraint has either one of the following forms. 

     - A procedure Constraint
       This constraint will be applied to the top-level score object of the CSP

     - A pair Constraint#Args, where Constraint is a procedure (applied to the top-level score object) and Args is a record of arguments given to P
       When calling the resulting parameterised script later, arguments for specific constraints are specified using the same record features (see explanation for arg Score above).

     - A pair Applicator#Constraint, where both Applicator and Constraint are procedures. The constraint is applied to the top-level score object with the given Applicator, which controls to which score objects the constraint is actually applied.

     - A nested pair (Applicator#ArgsA)#(Constraint#ArgsC), where both Applicator and Constraint are procedures, and ArgsA and ArgsC are both a record of arguments. 
       When calling the resulting parameterised script later, arguments for specific applicators and constraints are specified using the same record features (see explanation for arg Score above).

       The resulting script automatically supports for all constraints the argument value false which switches this constraint off completely.  



*** !! Required Constraint Applicators
    
    Some examples listed below

**** !!! Todo: Nested constraint applicators / combining constraint applicators

     Example: constraint applied to sim notes of consecutive parts (e.g., forbid voice crossing)

     Task: how-to translate the current approach sketched below into reasonably easy-to-define-and-use procedures that expect the top-level score and a constraint as arg
     
     Current approach: nested constraint applicator calls within proc given to another constraint applicator.

***** Simple example

     Simple example: ={MyPattern {{GetPart MyScore 1} map($ getPitch test:isNote)}}=

     (GetPart still undefined)

     I can always define constraint applicators ad-hoc as follows. However, I do not think this will be suitable for PWGL.

#+begin_src oz :results output :exports none
proc {$ MyScore MyConstraint} 
  {MyConstraint {{GetPart MyScore 1} map($ getPitch test:isNote)}}
end
#+end_src
     

     Attempt to translate this into applicator ForParameterList. Note: lengthy definition (though easy to define). Resulting applicator now fits format required by CSP generator, but it is rather inflexible. Also, is it really more simple to use than above example? The PWGL GUI could know the interface of the applicator, but then I have to make its definition even more complex..

#+begin_src oz :results output :exports none
  proc {ForParameterList MyScore MyConstraint Args}
     Defaults = unit(part:1
                     test:isNote
                     map:getPitch)
     As = {Adjoin Defaults Args}
  in
     {MyConstraint
      {{GetPart MyScore as.part} % GetPart yet undefined
       map($ As.map test:As.test)}} 
  end
  
  %% Usage example
  {ForParameterList MyScore MyPattern
   unit(part:1 test:isNote map:getPitch)}       
#+end_src

***** More complex example

     The following more complex example nests the following applicators: ForPartList, Pattern.forNeighboursInd, ForSimNotes, ForAll 


     NOTE: this is even only a simplified case where voice crossing is always forbidden between all consecutive voice pairs. 

#+begin_src oz :results output :exports none
  {ForPartList MyScore            % ForPartList yet undefined
   proc {$ Parts}
      {Pattern.forNeighbours Parts 2
       proc {$ [Part1 Part2]} 
          {ForSimNotes Part1 Part2
           proc {$ N1#N2s}
              thread 
                 {ForAll N2s
                  proc {$ N2}
                     {NoVoiceCrossing N1 N2} 
                  end}
              end
           end}
       end}
  ...
  proc {NoVoiceCrossing N1 N2}
     {N1 getPitch($)} >=: {N2 getPitch($)} 
  end
#+end_src

     Redefine above applicator as a single proc. Then think about ways to break the definition down. 

     Note: MyConstraint in the def below expects two notes -- this constraint applicator only works for that case!

#+begin_src oz :results output :exports none
  proc {MySpecialApplicator MyScore MyConstraint}
     {ForPartList MyScore            % ForPartList yet undefined
      proc {$ Parts}
         {Pattern.forNeighbours Parts 2
          proc {$ [Part1 Part2]} 
...
#+end_src
     ...
     *Once I have a working solution think about how to simplify it for non-experienced users*


***** ? Idea for solution: include applicator in constraint definition

      I could move all the complexities of the constraint applicator into the actual constraint definition, which is then applied to the full score, possibly with args to control, say, to which voice pairs the constraint is applied

      This is actually the approach of PWGL -- couple constraint and its application

      Advantages
      - Constraint is easy to use
      - No need to invent another complex constraint application mechanism


      Disadvantages
      - Whole constraint application approach becomes very inflexible -- possibly suitable for inexperienced users (like my students), but not for myself
	- For certain constraints like the present NoVoiceCrossing this approach may actually be suitable, for other constraints where I want to experiment likely less so.
	- !! E.g., if I want to constraint the sequence of pitches of a part by some predefined Pattern I currently define a nested applicator with, say, ={MyPattern {MyPart map($ getPitch test:isNote)}}=
      - Constraints become more complex to define
      - I have to define all these extended constraints -- currently all predefined constraints are very light-weight
...
**** {ForParts MyScore P}
     ...
     Applies P to every part, i.e., every quasi top-level seq in MyScore that does only contain notes, rests and other temporal containers (i.e. no chords, scales, or measure objects)

**** {ForParts2 MyScore P Args}

     Args is a record that allows to specify to which part P is applied (and perhaps some more things)


**** {ForPartList MyScore P}

     Applies P to list of all parts in MyScore


**** {ForNotes MyScore P}

     Applies P to every note in MyScore


**** {ForNoteList MyScore P}

     Applies P to list of all note in MyScore


**** ForSimNotes

     See [[file:~/Compositions/Orgel-StPeter-Koeln/Strasheela-source/Rules.oz::proc%20{ForSimNotes%20Part1%20Part2%20P}][file:~/Compositions/Orgel-StPeter-Koeln/Strasheela-source/Rules.oz::proc {ForSimNotes Part1 Part2 P}]]


**** Various other predefined contexts for SMapping.forContexts 

     See [[file:~/oz/music/Strasheela/strasheela/strasheela/source/ScoreMapping.oz::proc%20{ForContexts%20Xs%20GetContext%20P}][file:~/oz/music/Strasheela/strasheela/strasheela/source/ScoreMapping.oz::proc {ForContexts Xs GetContext P}]]


**** ?? Full support for PWConstraints pattern matching language, so I can use Jacopos predefined constraints :ARCHIVE:
     
     Not a good idea -- I cannot support the Lisp syntax of PWConstraints of the actual constraints 



*** Notes on usage

    Standard CSP templates can be defined using this generator, e.g., a counterpoint of multiple parts/voices (subscripts) with a simultaneous harmony and metric structure. The Score are for such a template expects a record/list of the individual parts, the harmony etc. 
    All relations between these score layers will be defined by constraints.
    Alternatively, the score with predefined constraints between its layers is given as a function to the template.


    Normally, the resulting script will be given directly to the solver.
    Only advanced users should specify arguments for to the resulting script

    

    I should write some tutorial documentation that focusses on explaining the more simple cases. More complex cases are then only documented by the reference (or perhaps better in one of more extra tutorial sections clearly indicated for advanced users)



*** Implementational notes 

    - !! Resulting script must support Score.getDefaults

    - Wrap each constraint applicator into its own thread


** TODO Port Arno constraint in-critical-band? to Strasheela
   
   see [[file:~/lisp/own-projects/cm-etc/arno/privat-examples/microtonal-choral.lisp::(defmethod%20in-critical-band?%20((chord%20abstract-chord))][file:~/lisp/own-projects/cm-etc/arno/privat-examples/microtonal-choral.lisp::(defmethod in-critical-band? ((chord abstract-chord))]]


** TODO Improving debugging of CSPs -- collect ideas

   Currently only brain storming..  

   What kind of information is actually desired?
   I cannot directly know which constraints caused failure, I can only observe their effect.
  
 
*** Report search statistics (e.g., from within Explorer or after calling/stopping search)
      
    - What kind of parameter failed most often (e.g., note pitch class)?
      -> Is that actually informative?

    - How often did it have to redo parameters for certain items (e.g., at what "time slice" did the search spend most time)



** TODO Finish support for grace notes
   ...
   See
   Fomus def:
   [[file:~/oz/music/Strasheela/strasheela/strasheela/source/Output.oz::EventClauses%20%3D%20{Append%20As.eventClauses][file:~/oz/music/Strasheela/strasheela/strasheela/source/Output.oz::EventClauses = {Append As.eventClauses]]
   Test:
   [[file:~/oz/music/Strasheela/strasheela/strasheela/examples/ControllingOutput-Examples/Fomus-Examples.oz::%25%25%20Grace%20notes:%20notes%20with%20duration%20%3D%200][file:~/oz/music/Strasheela/strasheela/strasheela/examples/ControllingOutput-Examples/Fomus-Examples.oz::%% Grace notes: notes with duration = 0]]
...
** TODO Revise Counterpoint_NParts definition
...
** TODO Add chord dissonance degree measure(s) to ScalaDB.oz   :NEXT:ARCHIVE:
...
** TODO Extend harmony model by support for voicing (pitch class octaves)
...
** TODO Finish scale databases for 12-TET in Scala.oz
...
** TODO Extend my harmony model so that chord definitions can take intervals beyond an octave into account
...
** TODO Define melody score script (flexible CSP template)
...
** TODO Define counterpoint script with arbitrary number of parts
...
** TODO [#B] Define interface to integrate Strasheela into OpenMusic or PWGL
...
** TODO Define/extend ENP score -> Strasheela score
...
** TODO [#B] Define Strasheela score transformations (instead of plain generation as so far)
  ...
  *I started to define such transformations in ENP*, see [[*%5B#A%5D%20Define%20ENP%20score%20transformations][Define ENP score transformations]]
  Do what you can in ENP and what is too hard in Strasheela
...
** TODO Define Strasheela scripts that expect score as input of which variation will be created
...
** TODO Debug  MIDI output of legato articulation
...
** TODO Add constrainable =marks= FS per note
...
** TODO Add constrainable =instruments= FS per note
...
** TODO Somehow add tempo statement to score
...
** TODO [Difficult] Generalise accessors hasTemporalPredecessor, hasTemporalSuccessor
...
** TODO Simplify modulation: apply modulation constraints automatically
...
*** TODO Collect examples for Strasheela tutorial
