...
<<AccentConstraints>>    
  ...
  %% */
  %% TODO: save in contributions/anders/HarmonisedScore/source/Output.oz
  ...
  %                          table:ET31.out.fomusPCs_Quartertones
                             getSettings:
                                fun {$ N}
                                   TestMarks = {VsToFomusMarks "r:"#{IsLongerThanSurrounding_Rated N}
                                                unit(where: 'x')}
                                in
                                   {AppendFomusMarks TestMarks {HS.out.makeNonChordTone_FomusMarks N}}
                                end)}
    ...
    {SDistro.exploreOne
     proc {$ MyScore}
        ...
        {ForAll {MyScore collect($ test:isNote)}.2 % !! NOTE: skip 1st note! 
         proc {$ N}
            {Accent_If N [IsLongerThanSurrounding_Rated] 
             unit(strictness: noteAndPosition % note position noteAndPosition
                  metricPosition: measureStart % Beat*2 beat accent measureStart
                  minRating: 2
                 )}
        ...
        {ForAll {MyScore collect($ test:Measure.isUniformMeasures)}
         proc {$ M}
            {NoteAtMetricPosition M {MyScore collect($ test:isNote)}
...
#+begin_src oz :results output
  ...
  {ForAll {MyScore collect($ test:Measure.isUniformMeasures)}
   proc {$ M}
      {SomeAtMeasurePosition M {MyScore collect($ test:isNote)}
       unit(metricPosition: measureStart)}
   end}
  ...
  {ForAll MeasureStarts
   proc {$ MyStart}
      thread
         SimNotes = {LUtils.cFilter {MyScore collect($ test:isNote)}
                     fun {$ N}
                        {AtTimeR N MyStart} == 1
                     end}
      in
