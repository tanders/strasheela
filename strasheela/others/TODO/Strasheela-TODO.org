...
*** TODO 
    ...
    - Examples in extra ./examples/ file (including output defs)
...
**** Example with reasonably low minRating of Accent_If, but additionally some ratings are constrained to be higher (with Pattern.howManyTrue where how many is at least a certain value)
...
TODO:
 ...
 - Add some overall pitch contour: some pattern on the local pitch maxima
 - Some additional accent constraints besides anacrusis?
...
#+begin_src oz :results output
    ...
    in
       MyScore = {Score.make sim([seq([note note note note note note note note
                                 ...
                                 timeUnit:beats(Beat))
                  add(note: fun {$ _}
                               {Score.make2 note(% duration: {FD.int Defaults.durDomain}
                                                 pitch: {FD.int 60#72})
                                unit(note:Measure.note)}
                            end
                      measure: Measure.uniformMeasures)}
