...
*** TODO 
    ...
    - Test with rests (offset times)

    - Predefiend accent constraints

    - Application examples

    - Copy these definitions later into the functor Measure.

    - Examples in extra ./examples/ file
...
#+begin_src oz :results output
  ...
  {Explorer.object
   add(information
       ...
       label: 'to Csound and Lily (with measures)')}
...
#+begin_src oz :results output
  ...
  declare    
    ...
    %%   position: every specified metric position has a note/item that meets the accent criteria, but there can also be such notes at other positions
    ...
    in
       thread
          ...
          = if {FD.is As.metricPosition}
            then
               proc {$ N ?B}
                  %% BUG: currently depends on uniform measure object start, not start time of individual measures. Probably need to use FD.modI...
                  B = ({FD.modI {N getStartTime($)} {SimMeasure getMeasureDuration($)}}
                       =: As.metricPosition)
                  % B = ({N getStartTime($)} =: {SimMeasure getStartTime($)} + As.metricPosition)
               end
            else 
    ...
    {SDistro.exploreOne
     proc {$ MyScore}
        ...
        {ForAll {MyScore collect($ test:isNote)}
         proc {$ N}
            {Accent_If N [IsQuarterNote] % IsLongerThanSurrounding
...
#+begin_src oz :results output
  ...
  MyScore = {Score.make sim([seq([note note note note])
                             seq([MyMeasure])]
                            startTime:0
                            timeUnit:beats(4))
             add(note: fun {$ _}
  ...
  {ForAll {MyScore collect($ test:isNote)}
   proc {$ N}
      {Accent_If N [IsLongerThanSurrounding]
       unit(strictness: position)}
   end}      
  
  
  {MyMeasure getAccents($)}
  ...
  {MyMeasure getBeatDuration($)}
  
  
  %% thinking..
  
%  B = {FS.reified.include Time-{ThisMeasure getStarTTime}
%       {MyMeasure getAccentsFS($)}}
...
*** Constraints supporting accents

    see my notes on paper and Structural Functions in Music, p. 339ff
...
#+begin_src oz :results output
  ...
  /** %%
  %% */
  %% Only direct neighbours so far
  fun {ApplyToPredecessors_Conj X P}
     Pre = {X getTemporalSuccessor($)}
  in
     if Pre == nil then 1
     else {P Pre X}
     end
  ...
  fun {ApplyToSuccessors_Conj X P}
     Succ = {X getTemporalPredecessor($)}
  in
     if Succ == nil then 1
     else {P Succ X}
     end
  ...
  %% Methods getPredecessors/getTemporalPredecessors getSuccessors/getTemporalSuccessor
  
  
  /** %%
  %% B=1 <=> Note N is longer than both its preceeding and its succeeding note. If a preceeding/succeeding note does not exist (in the same temporal container) then the condition is considered to be fulfilled.
  %% */
  %% 
  %% simplified version, see my notes
  %% TODO:
  %% - make more flexible, see my notes
  %% - better arg name than B
  fun {IsLongerThanSurrounding N}
     fun {IsShorter N1 N2} 
        ({N1 getDuration($)} <: {N2 getDuration($)})
     end
  in
     {FD.conj {ApplyToPredecessors_Conj N IsShorter}
      {ApplyToSuccessors_Conj N IsShorter}}
  end
  
  % proc {IsLongerNote_OLD N B}
  %    N1 = {N getTemporalPredecessor($ N)}
  %    N2 = N
  %    N3 = {N getTemporalSuccessor($ N)}
  % in
  ...
  /** %%
  %% */
  proc {IsHigherThanSurrounding N B}
     skip
  end
