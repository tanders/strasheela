...
**** TODO Compare current Strasheela output and desired output 
     ...
     - Avoid that convert-ly overwrites output (e.g., just disable it for a test), then create Lily output with Strasheela and keep a copy of the orig Strasheela output
     - Convert to current Lily version with convert-ly
     - Simplify convert-ly output (Lilypond then reduces the number of staffs) 
